...
Material ModelLoader::loadMaterial(aiMaterial* mat, const aiScene* scene, Texture* outAlbedo) {
	Material material = {};
...
Texture ModelLoader::loadMaterialTexture(aiMaterial* mat, aiTextureType type, const char* typeName, const aiScene* scene) {
	Texture texture = {};
...
Texture ModelLoader::loadTextureFromPath(const std::string& path, const char* typeName, ID3D11ShaderResourceView* fallback) {
	Texture texture = {};
	texture.type = typeName;
...
Texture ModelLoader::loadTextureBySuffix(const std::string& basePath, const std::vector<std::string>& suffixes, const char* typeName, ID3D11ShaderResourceView* fallback) {
	Texture texture = {};
	texture.type = typeName;
	texture.texture = fallback;
	texture.path = "";
...
ID3D11ShaderResourceView* ModelLoader::getDefaultWhiteTexture() {
...
ID3D11ShaderResourceView* ModelLoader::getDefaultNormalTexture() {
...
std::string ModelLoader::resolveTexturePath(const aiString& path) const {
