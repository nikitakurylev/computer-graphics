...
InputDevice::InputDevice()
{
	...
	Rid[0].hwndTarget = DisplayWin32::Instance().hWnd;

	Rid[1].usUsagePage = 0x01;
	Rid[1].usUsage = 0x06;
	Rid[1].dwFlags = 0;   // adds HID keyboard and also ignores legacy keyboard messages
	Rid[1].hwndTarget = DisplayWin32::Instance().hWnd;
	...
	DisplayWin32::Instance().OnKeyDown.AddRaw(this, &InputDevice::OnKeyDown);
	DisplayWin32::Instance().OnMouseMove.AddRaw(this, &InputDevice::OnMouseMove);
}
...
void InputDevice::OnMouseMove(RawMouseEventArgs args)
{
	...
	POINT p;
	GetCursorPos(&p);
	ScreenToClient(DisplayWin32::Instance().hWnd, &p);
